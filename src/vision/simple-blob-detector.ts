...
/**
 * This code is all from: https://gist.github.com/janpaul123/8b9061d1d093ec0b36dac2230434d34a
 */
...
function findBlobs(image, binaryImage, params) {
    ...
    for (let i = 0; i < contours.size(); i++) {
        ...
        if (params.filterByInertia) {
            if (params.faster) {
                throw new Error(
                    "Cannot both set params.faster and params.filterByInertia",
                );
            }
            ...
            if (
                ratio < params.minInertiaRatio ||
                ratio >= params.maxInertiaRatio
            )
                continue;
...
export default function simpleBlobDetector(image, params) {
        ...
        for (let i = 0; i < curCenters.length; i++) {
            ...
            for (let j = 0; j < centers.length; j++) {
                ...
                isNew =
                    ...
                    dist >=
                        centers[j][Math.floor(centers[j].length / 2)].radius &&
                ...
                if (!isNew) {
                    ...
                    while (
                        k > 0 &&
                        centers[j][k].radius < centers[j][k - 1].radius
                    ) {
    ...
    for (let i = 0; i < centers.length; i++) {
        ...
        let size = Math.round(
            centers[i][Math.floor(centers[i].length / 2)].radius * 2,
        );
        size = Math.min(
            size,
            sumPoint.x * 2,
